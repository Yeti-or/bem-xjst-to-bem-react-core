...
describe('Modes mods', function() {
  ...
  it('should return this.mods by default', function() {
    ...
    { content: [ { block: 'a' }, { block: 'b', mods: { type: 'promo' } } ] },
    '<div>{}{"type":"promo"}</div>');
  });

  xit('should apply templates for mods after mods changes', function() {
    test(function() {
      block('a')(
        mod('test', 'ok').tag()('span'),
        mods()({ test: 'ok' })
      );
    },
    { block: 'a' },
    '<span class="a a_test_ok"></span>');
  });

  xit('should support applyNext()', function() {
    test(function() {
      block('b')(
        mods()(function() {
